...
#define X_net_a 5   //goal position
#define Y_net_a -60
#define X_net_b 5
#define Y_net_b 85
#define X_assist_a 33   //waiting for passing ball position
#define Y_assist_a -30
#define X_pass_a 53     //passing ball position
#define Y_pass_a -30
#define X_assist_b -10
#define Y_assist_b 23
#define X_pass_b -40
#define Y_pass_b 28
#define X_home_a1 -70   //home position
#define Y_home_a1 0
#define X_home_b1 100
#define Y_home_b1 -3
#define X_home_a2  60
#define Y_home_a2  25
#define X_home_b2 -50
...
#define ir_thresh 70  //ir photodiode forwarding threshold
#define search_tol 900  //ir photodiode reaction threshold
#define defense_tol 100  //ir photodiode blocking ball threshold
#define angel_tol 3    //angel range
//state code
#define INIT 1
#define SEARCH 2
...
int x_init;
int y_init;
int id=4;  //identification of bots, 1/2 are snipers of team A, 3/4 are snipers of team B
char id_char;
int ad0;
int ad1;
...
//assign crucial coordinates and action flags 
void assign(int id){
	if(id==1){
		id_char='A';
	X_home=X_home_a1;
...
//LED API, identifier current state visually
...
//  printf ("rx_task PID=%d\r\n", nrk_get_pid ());
 ...
 //   printf ("]\r\n");
        ...
        m3pi.stop();
			...
			else if(rx_buf[2]=='4'&&actFlag!=4&&rx_buf[0]==id_char){   //4 for ball passed, try grab the ball now
	      led_set();
				m3pi.stop();
				wait_ms(1);
				led_clr();
     ...
     //6 for dejam, bots will backoff by a certain range to avoid jamming situations
      else if(rx_buf[2]=='6'&&(state==SEARCH||state==LOCALIZATION_A||state==LOCALIZATION_D)&&atoi(&rx_buf[3])!=current_cnt){
				m3pi.stop();
				wait_ms(20);
				m3pi.backward(0.4);
				if(jamCount%2)          //make backoff distance vary, so different bots will backoff differently, avoiding another jam
				wait_ms(id*150);
				else
				wait_ms((5-id)*150);
				jamCount++;
				...
				state=current_state;
				rx_buf[2]='99';
				current_cnt=atoi(&rx_buf[3]);
		//		printf("current cnt:%d\n",current_cnt);
			}
			//7 for game start, receive the signal along with current scores
			else if(rx_buf[2]=='7'&&inited==0){
				inited=1;
				goal_A=rx_buf[3];
				goal_B=rx_buf[4];
		//		printf("goal_a: %d, goal_b: %d\n",goal_A,goal_B);
				state=INIT;
			}
...
uint8_t ctr_cnt[4];
//turn transmit into module function, so no more redundant transmition
